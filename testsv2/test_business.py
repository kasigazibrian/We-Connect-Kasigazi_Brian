...
class FlaskTestCase(BaseTestCase):
    ...
    def test_API_can_obtain_a_specific_business(self):
        ...
        res = tester.post('/api/v2/businesses', data=json.dumps(self.businesses[0]))
        self.assertIn('business created successfully', str(res.data))
        res = tester.get('/api/v2/businesses/3')
    ...
    def test_API_can_get_all_businesses(self):
        ...
        res = tester.post('/api/v2/businesses', data=json.dumps(self.businesses[0]))
        self.assertIn('business created successfully', str(res.data))
        response = tester.get("/api/v2/businesses", data=json.dumps(self.businesses[0]))
    ...
    def test_API_will_not_update_business_without_token(self):
        ...
        tester = app.test_client(self)
        response = tester.post("/api/v2/businesses", data=json.dumps(self.businesses[0]))
        self.assertIn('business created successfully', str(response.data))
        result_in_json = json.loads(response.data.decode('utf-8').replace("'", "\""))
        res = tester.put('/api/v2/businesses/{}'.format(result_in_json['business_id']), data=json.dumps(self.businesses[0]))
        self.assertEqual(res.status_code, 200)
