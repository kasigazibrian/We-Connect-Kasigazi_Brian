...
class FlaskTestCase(unittest.TestCase):
    ...
    def test_API_can_obtain_a_specific_business(self):
        ...
        res = tester.get('/api/businesses/{}'.format(result_in_json['business_id']))
        self.assertEqual(res.status_code, 200)
        self.assertIn('alarm systems', str(res.data))
    ...
    def test_API_can_update_a_business(self):
        ...
        self.assertIn('business created successfully', str(response.data))
        result_in_json = json.loads(response.data.decode('utf-8').replace("'", "\""))
        res = tester.put('/api/businesses/{}'.format(result_in_json['business_id']), data=self.businesses[0])
        self.assertEqual(res.status_code, 200)
        self.assertIn('mediacom', str(res.data))

    def test_API_can_delete_a_business(self):
        """Tests if a business is deleted(to be modified)"""
        tester = app.test_client(self)
        response = tester.delete("/api/businesses/1")
        self.assertEqual(response.status_code, 200)
    ...
    def test_API_can_get_reviews(self):
        """Tests if a business review is added(to be modified)"""
        tester = app.test_client(self)
        response = tester.get("/api/businesses/1/reviews")
        self.assertEqual(response.status_code, 200)
        self.assertIn('Good job', str(response.data))
